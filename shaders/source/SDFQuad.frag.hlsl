...
float4 main(Input input) : SV_Target0
{
    ...
    float2 dis = coord - (input.model_pos - res_cam.zw);

    // define radius of circle
    float radius = min(input.model_scale.x / 2.0, input.model_scale.y / 2.0);
    ...
    float sa0 = input.arc_range[0];
    float ea0 = input.arc_range[1];
    float sa1 = input.arc_range[0];
    float ea1 = input.arc_range[1];
    if (sa0 == 0 && ea0 == 0) {
        sa0 = -PI;
        ea0 = PI;
    } else if (sa0 > ea0) {
        sa0 = -PI;
        ea0 = input.arc_range[1];
        sa1 = input.arc_range[0];
        ea1 = PI;
    }
    bool should_show = (ang > sa0 && ang < ea0) || (ang > sa1 && ang < ea1);

    if (len > radius || (input.thic > 0 && len < (radius - input.thic)) || !should_show) {
        a = 0;
    } else if (input.fade > 0) {
        float inner_edge = (radius - input.thic) + input.fade;
        if (len > inner_edge) {
            a = min((radius - len) / input.fade, 1);
        } else {
            a = 1.0 - min((inner_edge - len) / input.fade, 1);
        }
    }
    input.Color.a *= a;
